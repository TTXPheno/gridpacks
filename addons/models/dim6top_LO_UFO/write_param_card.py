...
class ParamCardWriter(object):
    ...
    def __init__(self, filename, referencepoint={}, list_of_parameters=None, generic=False):
        ...
        self.write_card(list_of_parameters, referencepoint)
        self.fsock.close()
    ...
    def write_card(self, all_ext_param, referencepoint):
        """ """
        ...
        for lhablock in all_lhablock:
            ...
            for param in need_writing:
                if lhablock == 'DIM6' and str(param) in referencepoint.keys():
                    param.value = float(referencepoint[str(param)])
...
if '__main__' == __name__:
    ...
    argParser.add_argument('--filename',       action='store', default="./param_card.dat", nargs=1,   type = str, help="Output filename")
    args = argParser.parse_args()
    
    if type(args.filename) == list: args.filename = str(args.filename[0])
    reference = ListToDict(args.referencepoint)

    ParamCardWriter(args.filename, referencepoint=reference, generic=True)
    print 'write %s' %(args.filename)
